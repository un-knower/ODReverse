 ...
 */
public class ODBackAfter extends EvalFunc<String> {
    ...
    public String exec(Tuple input)throws IOException {
        ...
        try{
            ...
            try{
                ...
                System.out.println("error!读取配置文件错误！");
                e.printStackTrace();
            }
            ...
            //接收参数
            String string0 =(String)input.get(0);
            String string1 =(String)input.get(1);
            String string2 =(String)input.get(2);
            String string3 =(String)input.get(3);
            String string4 =(String)input.get(4);
            String string5 =(String)input.get(5);
            String string6 =(String)input.get(6);
            String string7 =(String)input.get(7);
            String string8 =(String)input.get(8);
            String string9 =(String)input.get(9);
            String string10 =(String)input.get(10);

            String line =string0+","+string1+","+string2+","+string3+","+string4+","+string5+","+string6+","+string7+","+string8+","+string9+","+string10;
    ...
    public static void main(String[] args) {
        ...
        String path1 = "conf\\part\\part-m-00";
        Scanner scan1 = null;
        BufferedWriter bw = null;
        try {
            scan1 = new Scanner(new FileInputStream(new File(path1)), CodingDetector.checkTxtCode(path1));
            bw = new BufferedWriter(new FileWriter("conf\\part\\result_tos"));
        } catch (IOException e) {
            e.printStackTrace();
        }

        ODBackAfter test =new ODBackAfter();
        ...
        while (scan1.hasNext()){
            ...
            input.append(strings[1]);
            input.append(strings[2]);
            input.append(strings[3]);
            input.append(strings[4]);
            input.append(strings[5]);
            input.append(strings[6]);
            input.append(strings[7]);
            input.append(strings[8]);
            input.append(strings[9]);
            input.append(strings[10]);

            try {
                String result =test.exec(input).toString();
                ...
                if (strings_allpath2[0].equals("1")) {
                    ...
                    for (m = 0; m < value.size(); m++) {//56 57 #0 T 0.00000000  1.76666667
                        if (value.get(m).split("#")[1].split(" ")[0].equals("1")) {
                            ...
                            String changeStations = Route2ChangeStation(value.get(m).split("#")[0].trim().replaceAll(" ", "-"), linestation2staion);
...
//        System.out.println("string:"+string);
    ...
    public static String CloseTimeLine(List<String> value,double time,double realTime){
        ...
        for(int m =0;m<value.size();m++){//56 57 #0 T 0.00000000  1.76666667
            String[] times =value.get(m).split("#")[1].split(" ");
            if (Double.parseDouble(times[times.length-1]) <=realTime && Double.parseDouble(times[times.length-1]) >=closeTime){ ////  大于  改成  小于了   改回大于最接近的时间
                closeTime =Double.parseDouble(times[times.length-1]);
                closeLine =value.get(m);
                continue;
            }
